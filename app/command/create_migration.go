...
func CreateMigtaion(app *model.Create) {
	...
	if os.IsNotExist(err) {
		...
		writeMigration += "		CREATE TABLE IF NOT EXISTS " + app.TableName + "(\n"
		if os.Getenv("DB_DRIVER") == "mysql" {
			...
			writeMigration += "			id SERIAL,\n"
			writeMigration += "			name VARCHAR (225) NOT NULL,\n"
			writeMigration += "			created_at TIMESTAMP NOT NULL,\n"
			writeMigration += "			updated_at TIMESTAMP NOT NULL,\n"
			writeMigration += fmt.Sprintf("			CONSTRAINT %s_pkey PRIMARY KEY (%s_id)\n", app.TableName, app.TableName)
		}
		writeMigration += "		)\n"
		writeMigration += "	`\n"
